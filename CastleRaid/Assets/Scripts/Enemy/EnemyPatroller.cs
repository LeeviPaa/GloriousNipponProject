...
public class EnemyPatroller : MonoBehaviour
{
    ...
    RigidbodyConstraints defaultConstraints = RigidbodyConstraints.FreezeRotation & RigidbodyConstraints.FreezePositionY;
    ...
    {
        UpdateEnemyState(Time.fixedDeltaTime);
    ...
    {
        Debug.Log("SetAlertnessState: " + newState);
    ...
    private void TurnTowardsGivenDirection(Vector3 direction, float deltaTime)
    {
        ...
        Vector3 rotationStep = Quaternion.FromToRotation(transform.forward, direction).eulerAngles;

        if (deltaTime != -1 && rotationStep.sqrMagnitude > rotationStepMax * rotationStepMax)
        {
            rotationStep.Normalize();
            rotationStep *= rotationStepMax * deltaTime;
        }
    ...
    private bool RaycastForGivenTargetWithinVision(GameObject target)
    {
        ...
        if (Vector3.Angle(transform.forward, dir) <= (currentVisionAngle / 2))
        {
            ...
            {
                if (hit.collider.GetComponent<TargetableByEnemy>().GetMainObject() == target)
                {
                    return true;
                }
            }
